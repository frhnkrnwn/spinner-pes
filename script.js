...
$("start").onclick=()=>{
  ...
  const neededTotal=players.length*teamsPerPlayer;
  
  if(totalTeams<neededTotal){
    alert(`Tim tidak cukup. Butuh ${neededTotal} tim (${players.length} peserta × ${teamsPerPlayer} tim), tapi baru ada ${totalTeams} tim di semua tier.`);
    return;
  }
